"""Tool: get_ipv6_routes

Retrieve the IPv6 routing table via FRR's native JSON output.
Source: SSH 'vtysh -c "show ipv6 route [vrf X] json"'

Same schema as get_routes but for IPv6. Kept as a separate tool for
catalog clarity; the underlying parse is identical.
"""
...
def get_ipv6_routes(
    *,
    ...
    if vrf:
        cmd = f'vtysh -c "show ipv6 route vrf {vrf} json"'
    else:
    ...
    return {
        "summary": {
            ...
            "source": "ssh vtysh show ipv6 route json",
        },
        "routes": flat,
    }
